...
void CGameObject::ReleaseShaderVariables()
{
	...
	if (m_ppMaterials) m_ppMaterials[m_nMaterials]->ReleaseShaderVariables();
}
...
void CGameObject::randRotate(float y)
{
	XMMATRIX xmCurrentWorld = XMLoadFloat4x4(&m_xmf4x4World);
	XMMATRIX xmRotation = XMMatrixRotationY(XMConvertToRadians(y));
	XMMATRIX xmResult = xmRotation * xmCurrentWorld;
	XMFLOAT4X4 m_xmf4x4WorldResult;
	XMStoreFloat4x4(&m_xmf4x4WorldResult, xmResult);
	m_xmf4x4World = m_xmf4x4WorldResult;
}
...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) : CGameObject(0, 1)
{
	...
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/stonefield.dds", RESOURCE_TEXTURE2D, 1);
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/blue.dds", RESOURCE_TEXTURE2D, 2);
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/stonefield2.dds", RESOURCE_TEXTURE2D, 3);
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/HeightMap(Alpha).dds", RESOURCE_TEXTURE2D, 4);
