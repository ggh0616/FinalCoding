...
void CPlayer::Move(DWORD dwDirection, float fDistance, bool bUpdateVelocity)
{
	if (dwDirection)
	{
		...
		// checkCollisionHeightMap(xmf3Shift, bUpdateVelocity);
	}
}


void CPlayer::checkCollisionHeightMap(XMFLOAT3& xmf3Shift, bool bUpdateVelocity)
{
...
CCamera* CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	case THIRD_PERSON_CAMERA:
		...
		m_pCamera->GenerateProjectionMatrix(1.01f, 3000.0f, ASPECT_RATIO, 60.0f);
		m_pCamera->SetViewport(0, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT, 0.0f, 1.0f);
		m_pCamera->SetScissorRect(0, 0, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT);
		break;
	default:
...
void CAirplanePlayer::OnPlayerUpdateCallback(float fTimeElapsed)
{
	...
	float fHeight = pTerrain->GetHeight(xmf3PlayerPosition.x, xmf3PlayerPosition.z, bReverseQuad) + 7.0f;
	SetPlaySpeedChange(20.f);
	SetPlaySprite(false);
	if (xmf3PlayerPosition.y < fHeight)
	{
		SetPlaySprite(true);
		SetPlaySpeedChange(0.f);
		float fTotalDistance = fHeight + 7.0f - xmf3PlayerPosition.y;
		float fMoveSpeed = fTotalDistance / 5.0f; 
		float fYMovement = fMoveSpeed * fTimeElapsed;
		xmf3PlayerPosition.y += fYMovement * 10.f;
