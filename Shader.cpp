...
void CBillboardObjectsShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
	...
	ppTreeTextures[0]->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/sand.dds", RESOURCE_TEXTURE2D, 0);
...
//}
//
//void CSpriteShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
//{
//	CTexture* ppSpriteTexture[2];
//	ppSpriteTexture[0] = new CTexture(0, RESOURCE_TEXTURE2D, 0, 1);
//	ppSpriteTexture[0]->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Sprite1.dds", RESOURCE_TEXTURE2D, 0);
//	ppSpriteTexture[1] = new CTexture(1, RESOURCE_TEXTURE2D, 0, 1);
//	ppSpriteTexture[1]->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Sprite2.dds", RESOURCE_TEXTURE2D, 0);
//
//	CMaterial* ppSpriteMaterial[2];
//	ppSpriteMaterial[0] = new CMaterial();
//	ppSpriteMaterial[0]->SetTexture(ppSpriteTexture[0]);
//	ppSpriteMaterial[1] = new CMaterial();
//	ppSpriteMaterial[1]->SetTexture(ppSpriteTexture[1]);
//
//	CTexturedRectMesh* pSpriteMEsh = new CTexturedRectMesh(pd3dDevice, pd3dCommandList, 0, 15.f, 20.f, 0.f, 0.f, 0.f, 0. );
//
//	m_nObjects = 2;
//	m_ppObjects = new CGameObject * [m_nObjects];
//
...
//	CreateShaderVariables(pd3dDevice, pd3dCommandList);
//	CreateConstantBufferViews(pd3dDevice, m_nObjects, m_pd3dcbGameObjects, ncbElementBytes);
//	CreateShaderResourceViews(pd3dDevice, ppSpriteTexture[0], 0, 15);
//	CreateShaderResourceViews(pd3dDevice, ppSpriteTexture[1], 0, 15);
//
//	CMaterial* pMaterial = NULL;
//	CMesh* pMesh = NULL;
//
//	pMesh = pSpriteMEsh;
//	pMaterial = ppSpriteMaterial[0];
//
//	for (int j = 0; j < m_nObjects; j++)
//	{
//		if (pMesh && pMaterial)
...
//}
//
//void CSpriteShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, CPlayer* pPlayer, int nPipelineState)
//{
//	XMFLOAT3 xmf3CameraPosition = pCamera->GetPosition();
//	for (int j = 0; j < m_nObjects; j++)
//	{
